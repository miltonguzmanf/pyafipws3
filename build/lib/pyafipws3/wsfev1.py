...
class WSFEv1(BaseWS):
    ...
    def __analizar_errores(self, ret):
        ...
        if 'Errors' in ret:
            errores = ret['Errors'].get('Err')
            for error in errores:
                self.Errores.append("%s: %s" % (
                    error['Code'],
                    error['Msg'],
                ))
            self.ErrCode = ' '.join([str(error['Code']) for error in errores])
            self.ErrMsg = '\n'.join(self.Errores)
        if 'Events' in ret:
            events = ret['Events']
            self.Eventos = ['%s: %s' % (evt['Code'], evt['Msg']) for evt in events['Evt']]
    ...
    def CAESolicitar(self):
                ...
                'FeDetReq': {'FECAEDetRequest': [{
                    ...
                    'CbtesAsoc': f['cbtes_asoc'] and
                        {'CbteAsoc': [{
                            ...
                            'CbteFch': cbte_asoc.get('fecha_cbte'),
                        }
        ...
        if 'FeCabResp' in result:
            ...
            fedetresp = result['FeDetResp']['FECAEDetResponse'][0]
            ...
            # Obs:
            for obs in fedetresp.get('Observaciones', []) and fedetresp.get('Observaciones', []).get('Obs', []):
